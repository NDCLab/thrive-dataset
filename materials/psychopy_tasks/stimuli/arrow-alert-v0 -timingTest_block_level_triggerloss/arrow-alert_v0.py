...
expName = 'arrow-alert_v0'  # from the Builder filename that created this script
expInfo = {
    'id': '1',
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\NDCLab\\Desktop\\Experiments\\thrive\\arrow-alert-v0\\arrow-alert_v0.py',
...
# --- Initialize components for Routine "oberve_other" ---
...
# keep track of which components have finished
welcomeComponents = [text, welcome_text, welcome_keyResp]
...
prac_block_loop = data.TrialHandler(nReps=1, method='random', 
...
# completed 1 repeats of 'prac_block_loop'
...
for thisTask_condition_loop in task_condition_loop:
    ...
    # --- Prepare to start Routine "oberve_other" ---
    continueRoutine = True
    routineForceEnded = False
    # update component parameters for each repeat
    # Run 'Begin Routine' code from observe_other_code
    if conditionText == 'Observe other':
        task_block_loop.finished = True
    # keep track of which components have finished
    oberve_otherComponents = []
    for thisComponent in oberve_otherComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Ending Routine "oberve_other" ---
    for thisComponent in oberve_otherComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # the Routine "oberve_other" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
